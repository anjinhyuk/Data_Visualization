...
public class MainPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void paintComponent(Graphics g1) {
		...
		for (HyrumPolyline p : lines) {
			if(p.getSelected() && p.getSelectable()) {
				if(dragged) {
					p.setLineColor(Color.CYAN);
					p.setStrock(new BasicStroke(2));
				} else {
					p.setLineColor(Color.BLACK);
					p.setStrock(new BasicStroke(1));
				}
				p.draw(g);
			} else {
		...
		if(highlighted != null) {
			highlighted.setLineColor(Color.CYAN);
			highlighted.setStrock(new BasicStroke(5));
			highlighted.draw(g);
			highlighted = null;
		}
	...
	public void mouseDragged(MouseEvent e) {
		int x = e.getX();
		int y = e.getY();
		rect.setFrameFromDiagonal(mouseDown.x, mouseDown.y, x, y);
		for(HyrumPolyline hp : lines) {
			Shape sh = hp.getShape();
			if(sh.intersects(rect) && hp.getSelectable() == true) {
				hp.setSelected(true);
				if(hp.getSelectable()) {
	...
	//When the mouse released, choose the size of rect
	@Override
	public void mouseReleased(MouseEvent e) {
		int x = e.getX();
		int y = e.getY();
		Point mousePos = new Point(x,y);
		rect.setFrameFromDiagonal(mouseDown.x, mouseDown.y, mousePos.x, mousePos.y);
		rect = null;
